        ...
        else
        {
            if (!isCatching)
            {
                ...
                if (t >= 1f)
                {
                    ...
                    followTimer = 0;
                    iTween.MoveTo(currentFish, iTween.Hash("x", boat.transform.position.x, "y", boat.transform.position.y + 2, "z", boat.transform.position.z, "time", 1, "easeType", iTween.EaseType.spring));
